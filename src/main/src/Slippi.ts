...
export default class Slippi {
    ...
    run() {
        ...
        this.watcher.on('change', (path) => {
            ...
            _.forEach(settings.players, player => {
                ...
                this.checkForDPad(frameData, player.playerIndex)
    ...
    checkForDPad(frameData: any, playerIndex: string | number) {
        if (this.playerIsPressingButton("A", frameData)) {
            this.obs.setCurrentScene("ZoomScene");
            setTimeout(() => {
                this.obs.setCurrentScene("Melee");
              }, 2000);
        }
    }
