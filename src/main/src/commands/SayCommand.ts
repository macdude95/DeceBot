...
export default class SayCommand extends Command {
  ...
  public execute = async (user: ChatUserstate, message: string) => {
    ...
    const maxSayDuration = this.sayLengthPermissions.hasOwnProperty(username)
      ? this.sayLengthPermissions[username]
      : config.sayTimeout;
    ...
    if (maxSayDuration > 0) {
      cutoffMessage = setTimeout(() => {
        say.stop(err => {
          if (!err && speaking) {
            sayInChat(
              `@${username} your message was too long so I had to cut off.`
            );
          }
        });
      }, maxSayDuration);
  ...
  private parseCommand = (rawText: string): string => {
    return speechAssistReplacements
      .reduce(
